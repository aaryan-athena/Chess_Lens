...
const fetchResponse = async (token: string, path: string, options: any = {}) => {
  ...
  const res: any = await window.fetch(`${cchessHost}${path}`, config);
  if (!res.ok) {
...
export const cchessLogin = () => {
  const oauth = getOauth();
  oauth.fetchAuthorizationCode();
}

export const cchessLogout = (dispatch: Dispatch<AnyAction>) => {
  localStorage.removeItem("oauth2authcodepkce-state");
  dispatch(userReset());
}

export const cchessGetAccount = (token: string) => {
  const path = "/api/account";
  const account = fetchBody(token, path);
  return account;
}

export const cchessSetStudies = (token: string, setStudies: any, username: string, onlyBroadcasts: boolean) => {
...
export const cchessImportPgn = (token: string, pgn: string) => {
...
export const cchessImportPgnToStudy = (token: string, pgn: string, name: string, studyId: string) => {
...
export const cchessPushRound = (token: string, pgn: string, roundId: string) => {
...
export const cchessStreamGame = (token: string, callback: any, gameId: string) => {
  const path = `/api/board/game/stream/${gameId}`;
  fetchResponse(token, path)
  .then(readStream(callback));
}

export const cchessGetPlaying = (token: string) => {
  const path = "/api/account/playing";
  const playing = fetchBody(token, path);
  return playing;
}

export const cchessPlayMove = (token: string, gameId: string, move: string) => {
...
export const cchessTrySetUser = async (navigate: NavigateFunction, dispatch: Dispatch<AnyAction>) => {
  ...
  const account: any = await cchessGetAccount(newToken);
